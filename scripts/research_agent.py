...
_NOTES_RULES = """## Editor Notes (Jonathan's own observations — read these five rules before using any of them)
  ...
  4. **Notes are point-in-time observations, not current form, and they DECAY.** Every in-season note says how many games ago its anchor game now is. A note on the MOST RECENT game is a current observation and can be used as one. Two or more games back it is background only: the team may have changed since, so its figures must never be written in the present tense, and a characterization it makes ("committed to the run game", "the pass defense is a problem") must not be restated as though it still describes the team. Three or more games back, use it only where your sources independently say the same thing is still true today. The Current-Season Stats block is the authority for anything season-to-date; where a note and that block disagree, the block wins.
  5. **Preseason notes are PROJECTION**, written before a snap was played, and they decay fastest of all: they describe who the team was EXPECTED to be, are worth little once a few games have been played, and are never an observation of how the team is actually playing. Where a preseason note and any in-season evidence disagree, the in-season evidence wins outright — a projection that the season has already contradicted is simply dropped, not reconciled. `[!]` marks a note flagged as important.
"""
