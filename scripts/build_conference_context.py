    ...
    return [
        {
            ...
            'returning_offense_pct':          c.get('returning_offense_pct'),
            'returning_defense_pct':          c.get('returning_defense_pct'),
            'returning_production_rank':      c.get('returning_production_rank'),
            'returning_offense_rank':         c.get('returning_offense_rank'),
            'returning_defense_rank':         c.get('returning_defense_rank'),
            'billc_returning_production_pct': c.get('billc_returning_production_pct'),
            'billc_returning_offense_pct':    c.get('billc_returning_offense_pct'),
            'billc_returning_defense_pct':    c.get('billc_returning_defense_pct'),
...
def build_marquee_ooc(conn, conf_games_name, season=SEASON, max_games=10):
    ...
    sql = f"""
        ...
        -- k_dyn = 0.75 + 0.02 * MAX(0, 5 - avg_rating) gives a flat 0.75
        -- penalty weight above avg=5 and ramps up for weaker matchups, so
        -- lopsided games (e.g. Texas vs UTSA) don't outrank close marquee
        -- games (e.g. Florida vs Auburn). See repo notes for tuning levers.
        ORDER BY (
            (COALESCE(hp.rating, 0) + COALESCE(ap.rating, 0))
            - (0.75 + 0.02 * GREATEST(0, 5 - (COALESCE(hp.rating, 0) + COALESCE(ap.rating, 0)) / 2.0))
