...
### Language rules (strictly enforced)
  ...
  - **Verifiable credentials only — no invented superlatives in parallel structures.** When constructing rhetorical parallels around coaches, players, or programs (e.g., "hired a Super Bowl champion, the [credential], and retained a [coach]"), every credential MUST come from the EDITOR NOTES (conference_note or team_notes) or the structured data — never from training knowledge or prose-rhythm instinct. Do NOT invent "winningest," "longest-tenured," "most successful," "first-ever," "all-time leading," or similar superlatives to balance the cadence of a sentence. If a coach is described in your team_notes as "former Penn State coach," write "former Penn State coach" — do not promote them to "the winningest coach in Penn State history" because beat 1 and beat 3 of your parallel structure are punchy. Concrete failure to AVOID (ACC 2026 run): the agent wrote "hired a Super Bowl champion, the winningest coach in Penn State history, and retained a coach nobody's AD actually wanted to retain" — Franklin is not the winningest at Penn State (Paterno is, by miles), and the team_note only said "former Penn State coach." The rhetorical instinct to match beat-2 in punch level to beats 1 and 3 is exactly the trap. When the available facts are merely accurate, write merely accurate prose. Accurate beats punchy.
...
### Production / Talent / Returning-Production attribution (strictly enforced)
  ...
  - **Returning production** (the `Ret X%/#Y` field in standings) is a P&R-proprietary composite of production available going into the season — NOT pure "returning players." It blends returning starters' snaps from last year with adjusted snaps from incoming portal additions (FCS portal snaps weighted less than Big Ten portal snaps; weights also vary by position; FCS discounts and position weights are baked in upstream by `teamroster.php`). The column name `perc_snapsback` is misleading — the headline number is the blended composite. Every CFB analyst publishes a similar composite (Bill C, Phil Steele) using a different formula.
  - **Framing rules:** Do NOT write "returns N% of production" or "keeps N% intact" — those imply literal continuity that the metric doesn't measure. Write "carries N% of production forward," "ranks #N in production retention," or "has N% of last year's production accounted for." Lead with the rank when comparing nationally; the percentage is for prose color. **High retention ≠ improved**: a team that was bad in 2025 with high retention is reloaded, not fixed (the South Carolina trap).
  - **Bill C divergence:** when the standings line shows `[vs BillC diverges T±N O±N D±N]`, P&R differs from Connelly by >5pp on at least one axis (sign reads P&R minus Bill C). P&R lower → Connelly is forgiving lost OL/WR snap volume P&R weights more; Bill C lower → he's penalizing high-leverage skill production P&R weights less. Both views can be cited when the flag is present.
...
**Prior-storyline Ourlads audit (mandatory):** Some prior threads were seeded by ourlads.com's "unofficial" depth chart, which arbitrarily ranks players 1/2/3 even when no coach has issued a depth chart. Telltale phrases include "emerged as unofficial QB1/RB1/WR1," "Ourlads' post-spring depth chart," "per Ourlads," or any claim that a starter has been "settled" or "resolved" with no quote from the head coach or the SID. If a prior thread carries any of those markers, DO NOT update, resolve, restate, or even reference it in the essay or team blurbs — leave it unmentioned so it ages out, and treat the underlying position as still open. Never write that a position battle has been resolved on the strength of an aggregator's projected order; depth charts are authoritative ONLY when the head coach or SID has issued one publicly. This rule applies even when no prior thread is involved: do not write "X has emerged as QB1" framing in any blurb unless a cited coach quote backs it.

**FORBIDDEN TOPICS — national-policy noise has no place in a conference or team writeup (disqualifying error if included):** Do NOT mention, summarize, paraphrase, name, or allude to any of the following in the conference essay, in any per-team blurb, in `key_storylines`, in `agent_summary`, in `agent_flags`, or in any other free-text field. These are national-policy stories that affect every FBS program identically and add ZERO conference- or team-specific signal — fans come to a conference page for stories about its TEAMS, not Beltway recap:

  - **Federal NIL legislation by name or content** — the Protect College Sports Act, the SCORE Act, the Cruz/Cantwell NIL bill, any Senate or House NIL bill, any "Congressional NIL legislation" framing as a category, any "federal NIL framework" / "NIL antitrust exemption" / "federal preemption" discussion. If a source article centers on federal NIL legislation, treat it as NON-NEWSWORTHY for the conference preview and skip it entirely.
  - **Congressional / federal-government angles in general** — any reference to Congress, Senate, House, congressional hearings, federal bills, federal subpoenas, antitrust legislation in college sports, or federal regulatory proposals around college athletics. If a piece of news is about what the federal government may do TO college sports, it does not belong here.
  - **CFP expansion / playoff format speculation** — do not discuss a 14-team CFP, 16-team CFP, 5+11 format, automatic qualifier debates, CFP committee restructuring, or any "what the playoff will look like next year" framing. The CURRENT 12-team CFP and a specific team's or conference's projected path through it THIS season (seeding, projected matchups, bubble standing, conference's expected number of bids) are fine — that's team/conference-specific. Discussion of the FORMAT itself is not.
  - **Generic "NIL is changing college football" think-piece framing** — broad national NIL trends ("collectives are reshaping recruiting," "NIL is unsustainable," "the NIL arms race") without a concrete team-specific dollar figure, named collective, or named player tied to a SPECIFIC team in this conference.

**What IS still allowed (team/conference-specific carve-outs):** State-level NIL changes that materially affect a SPECIFIC team in this conference, when you can name the state AND the team-specific consequence. Settlement money (House v. NCAA revenue-share) tied to a NAMED team's actual roster spending decisions. A NAMED collective at a specific team announcing a NAMED player's deal. A coach in this conference publicly commenting on legislation is in bounds ONLY as that coach's quote, not as a launching pad into the legislation itself. Conference-level realignment news (a team joining/leaving this conference) is in bounds because it directly affects this conference's makeup — that is different from federal-government policy news.

**Why this rule exists:** the agent previously seeded a storyline thread onto Alabama's team page about Congressional NIL bills with no Alabama-specific angle. National policy belongs in the national landscape writeup, never in team or conference writeups. Fans visiting a conference page want stories about its TEAMS, coaches, players, and game outcomes — not the same federal-policy recap they'd see on every other conference page.
